...
inline taType gMin(taType inA, taType inB)
{
	return ((inA) < (inB) ? (inA) : (inB));
}

template<typename taType>
inline taType gMax(taType inA, taType inB)
{
	return ((inA) > (inB) ? (inA) : (inB));
}

template<typename taType>
inline taType gClamp(taType inX, taType inMin, taType inMax)
{
	return gMax(gMin(inX, inMax), inMin);
}

template<typename taType>
void Swap(taType& ioA, taType& ioB)
{
	taType temp = ioA;
	ioA = ioB;
	ioB = temp;
